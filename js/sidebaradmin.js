...
export async function loadSidebar() {
    try {
        const response = await fetch('sidebar-admin.html');
        if (!response.ok) {
        ...
        // Event listener for Students item
        const studentsItem = document.querySelector('.sidebar-item:nth-child(3)');
        const subItems = document.querySelector('ul');

        if (studentsItem && subItems) {
        ...
        onAuthStateChanged(auth, async (user) => {
            if (user) {
                ...
                // Retrieve admin profile info from Firestore using the UID
                const adminRef = doc(db, 'admins', userUid); // Use UID as document ID
                const adminDoc = await getDoc(adminRef);

                if (adminDoc.exists()) {
                    const adminData = adminDoc.data();
                    const profilePicPath = adminData.profilePicture;
                    ...
                    console.log("Admin profile does not exist.");
                }
            } else {
